...
for i, items in enumerate([
    ...
    recom_valid_t1_df = recall_feat_func(pd.concat(item1+[valid_qrel_t2], axis=0), valid_run_t1, str(i))
    recom_test_t1_df  = recall_feat_func(pd.concat(item1+[valid_qrel_t1,valid_qrel_t2], axis=0), test_run_t1, str(i))

    recom_valid_t2_df = recall_feat_func(pd.concat(item2+[valid_qrel_t1], axis=0), valid_run_t2, str(i))
    recom_test_t2_df  = recall_feat_func(pd.concat(item2+[valid_qrel_t1,valid_qrel_t2], axis=0), test_run_t2, str(i))
    
    recom_valid_t1_df.to_csv('./recall_feature_data/recom_valid_t1_feat{}.csv'.format(str(i)), index=False)
    recom_test_t1_df.to_csv('./recall_feature_data/recom_test_t1_feat{}.csv'.format(str(i)), index=False)

    recom_valid_t2_df.to_csv('./recall_feature_data/recom_valid_t2_feat{}.csv'.format(str(i)), index=False)
    recom_test_t2_df.to_csv('./recall_feature_data/recom_test_t2_feat{}.csv'.format(str(i)), index=False)