...
def xgb_model(x_train, y_train, x_test, seed):
    xgb_train, xgb_test = cv_model(xgb, x_train, y_train, x_test, "xgb", seed)
...
for sd in random_list:
    lgb_train, lgb_test = xgb_model(valid_t1_df[cols], valid_t1_df['label'].astype(int), test_t1_df[cols], sd)
...
for sd in random_list:
    lgb_train, lgb_test = xgb_model(valid_t2_df[cols], valid_t2_df['label'].astype(int), test_t2_df[cols], sd)
...
def mkdir(d):
    if not os.path.exists(d):
        os.makedirs(d)
mkdir('./merge/result_xgb/t1/')
mkdir('./merge/result_xgb/t2/')
              
#### 提交结果
### t1
test_t1_df['score'] = lgb_test_t1
test_t1_df = test_t1_df.sort_values(['userId','score'], ascending=False)
test_t1_df[['userId','itemId','score']].to_csv('./merge/result_xgb/t1/test_pred.tsv', sep='\t', index=False) 

valid_t1_df[['userId','itemId','score']].to_csv('./merge/result_xgb/t1/valid_pred.tsv', sep='\t', index=False)

### t2
test_t2_df['score'] = lgb_test_t2
test_t2_df = test_t2_df.sort_values(['userId','score'], ascending=False)
test_t2_df[['userId','itemId','score']].to_csv('./merge/result_xgb/t2/test_pred.tsv', sep='\t', index=False)

valid_t2_df[['userId','itemId','score']].to_csv('./merge/result_xgb/t2/valid_pred.tsv', sep='\t', index=False)